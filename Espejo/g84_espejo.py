...
Version 0.4-dev          16/noviembre/2025
...
Funciones Añadidas:
Ver. 0.4 - Ahora el espejo se inicializa (saca) al iniciar el programa. Fix de memory leak en ram.
Ver. 0.3 - Siempre se quedara energizado el puente H porque ahora usamos switch duros.
...
# ADAFRUIT MOTORHATS
# hat is default address 0x60
bottomhat = Adafruit_MotorHAT(addr=0x60)
ESPEJO_MOTOR = bottomhat.getMotor(1)  # Motor 1 para el espejo
# set the speed to start, from 0 (off) to 255 (max speed)
ESPEJO_MOTOR.setSpeed(160)
ESPEJO_TIMEOUT = 8  # segundos para el timeout
...
def meteEspejo():
    ...
    if GPIO.input(METE_PIN) == GPIO.LOW:
        ...
        print ("[+] avanza Espejo")
        time.sleep(0.20)
        ESPEJO_MOTOR.run(Adafruit_MotorHAT.FORWARD)
        variables.ESPEJO_STOP = 0
...
def sacaEspejo():
    ...
    if GPIO.input(SACA_PIN) == GPIO.LOW:
        ...
        time.sleep(0.20)
        ESPEJO_MOTOR.run(Adafruit_MotorHAT.BACKWARD)
        variables.ESPEJO_STOP = 0
...
class Principal():
    ...
    def ESPEJO_METIDO(self, channel):    #switch normalmente abierto
        time.sleep(0.005)  # debounce 0.005 original
        if GPIO.input(METE_PIN) == GPIO.LOW:
            variables.ESPEJO_METIDO = 1
            variables.ESPEJO_ESTADO = "METIDO"
